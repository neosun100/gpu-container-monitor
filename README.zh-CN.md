...
![GPU 容器监控仪表板](images/dashboard-screenshot.png)

*实时 GPU 监控，界面美观现代 - 一目了然地追踪温度、功耗、利用率和显存使用情况*
...
- 📦 **容器管理** - 完全控制使用 GPU 的容器
  ...
  - 彩色状态指示器查看容器状态
  - 查看每个容器使用的 GPU
  - 自动检测使用 `NVIDIA_VISIBLE_DEVICES=all` 的容器
  
- 🎨 **精美界面** - 现代化、响应式 Web 界面
  - 渐变色主题
  - 实时更新（1秒/3秒/5秒/10秒间隔，默认：1秒）
  - 实时时钟显示，验证页面响应状态
  - 状态图例快速参考
  - 移动端友好设计
...
- 🐛 修复使用 `NVIDIA_VISIBLE_DEVICES=all` 的容器检测问题
- ⚡ 将默认刷新间隔从 3 秒改为 1 秒，提升响应速度
- 🎯 改进 GPU 设备检测逻辑，支持各种 Docker 配置
...
- 问题反馈：[GitHub Issues](https://github.com/neosun100/gpu-container-monitor/issues)
