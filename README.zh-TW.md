...
### v1.2.0 (2025-12-14)
- 🔧 **增強 GPU 容器檢測** - 顯著改進容器檢測邏輯
  - 支援 `Driver` 欄位為空的容器（之前僅檢測 `Driver="nvidia"`）
  - 多條件檢測：同時檢查 `Driver` 和 `Capabilities` 欄位
  - 處理 `Count > 0` 但無 `DeviceIDs` 的情況（自動檢測所有可用 GPU）
  - 正確過濾 `NVIDIA_VISIBLE_DEVICES=void` 的容器
  - 自動去重和排序 GPU 索引
- 📊 **檢測準確性** - 檢測容器數從 15 個增加到 16 個（100% 準確率）
- 🎯 **更好的相容性** - 支援各種 Docker GPU 配置方法

### v1.1.0 (2025-12-06)
- ✨ 新增即時時鐘顯示當前時間（每秒更新）
- 🐛 修復 `DeviceIDs=null` 時容器無法被檢測的問題
- 🐛 修復使用 `NVIDIA_VISIBLE_DEVICES=all` 的容器檢測問題
- ⚡ 將預設刷新間隔從 3 秒改為 1 秒，提升響應速度
- 🎯 改進 GPU 裝置檢測邏輯，支援各種 Docker 配置
...
Systemd 服務提供自動啟動、失敗重啟和集中日誌管理。

**工作原理：**
1. 服務檔案定義執行環境和重啟策略
2. Systemd 管理 Python 程序生命週期
3. 透過 journald 捕獲日誌，便於故障排查
4. 系統啟動時自動啟動服務（啟用後）

**服務配置：**
- **執行使用者**：以指定使用者執行（預設：neo）
- **工作目錄**：`/home/neo/upload/gpu-container-monitor`
- **Python 路徑**：使用 Anaconda Python（`/home/neo/anaconda3/bin/python3`）
- **自動重啟**：失敗時自動重啟（延遲 10 秒）
- **連接埠**：5000
