...
### v1.2.0 (2025-12-14)
- 🔧 **GPUコンテナ検出の強化** - コンテナ検出ロジックを大幅に改善
  - `Driver`フィールドが空のコンテナをサポート（以前は`Driver="nvidia"`のみ検出）
  - 複数条件検出：`Driver`と`Capabilities`フィールドの両方をチェック
  - `DeviceIDs`なしで`Count > 0`を処理（利用可能なすべてのGPUを自動検出）
  - `NVIDIA_VISIBLE_DEVICES=void`のコンテナを正しくフィルタリング
  - GPUインデックスの自動重複排除とソート
- 📊 **検出精度** - 検出コンテナ数が15から16に増加（100%精度）
- 🎯 **互換性の向上** - さまざまなDocker GPU設定方法に対応

### v1.1.0 (2025-12-06)
- ✨ 現在時刻を表示するライブクロックを追加（毎秒更新）
- 🐛 `DeviceIDs=null`のコンテナが検出されないバグを修正
- 🐛 `NVIDIA_VISIBLE_DEVICES=all`を使用するコンテナの検出を修正
- ⚡ デフォルトの更新間隔を3秒から1秒に変更し、応答性を向上
- 🎯 さまざまなDocker設定に対応するGPUデバイス検出ロジックを改善
...
Systemdサービスは、自動起動、障害時の再起動、集中ログ管理を提供します。

**動作原理：**
1. サービスファイルが実行環境と再起動ポリシーを定義
2. SystemdがPythonプロセスのライフサイクルを管理
3. journaldを介してログをキャプチャし、トラブルシューティングを容易化
4. システム起動時にサービスを自動起動（有効化時）

**サービス設定：**
- **実行ユーザー**：指定されたユーザーとして実行（デフォルト：neo）
- **作業ディレクトリ**：`/home/neo/upload/gpu-container-monitor`
- **Pythonパス**：Anaconda Python（`/home/neo/anaconda3/bin/python3`）を使用
- **自動再起動**：障害時に自動再起動（10秒遅延）
- **ポート**：5000
